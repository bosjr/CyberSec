# CYBERSEC Hub

*Plateforme centralisée regroupant mes ressources, projets et travaux pratiques en cybersécurité.*

| Section | Contenu |
|--------|--------|
| [Cours](https://github.com/bosjr/CyberSec-cours) | Cours |
| [CTF](https://github.com/bosjr/CyberSec-ctfs) | Write-ups |
| [Docs](https://github.com/bosjr/CyberSec-docs) | Docs |
| [Labs](https://github.com/bosjr/CyberSec-labs) | Labs |
| [Outils](https://github.com/bosjr/CyberSec-outils) | Outils |
| [Projets](https://github.com/bosjr/CyberSec-projets) | Projets |
| [Ressources](https://github.com/bosjr/CyberSec-ressources) | Ressources |

## Objectif

Ce dépôt sert de point d’entrée unique vers :

des projets pédagogiques
des laboratoires (labs)
des write-ups CTF
des supports de cours
...

Chaque élément est isolé dans un dépôt dédié afin de garantir :

une meilleure organisation
un versioning indépendant
une réutilisation simplifiée

## Utilisation pédagogique

Ce hub est utilisé dans le cadre de formations en cybersécurité pour :

guider les étudiants
centraliser les accès aux TP
structurer les parcours d’apprentissage

Auteur : [Bosjr](https://www.linkedin.com/in/bosjr)

Formateur en cybersécurité
Spécialités :
SOC / SIEM
Pentest (niveau initiation/intermédiaire)
Forensique
EBIOS RM

## Licence
- Code et scripts : **MIT License**  
- Contenu écrit : **CC0‑1.0 (Public Domain)**
